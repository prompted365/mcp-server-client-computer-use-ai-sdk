...
impl MacOSEngine {
    ...
    pub fn scroll_at_position(
        ...
        // Set the mouse position first (optional)
    ...
    pub fn scroll_at_current_position(
        ...
        // Log current attempt at getting mouse location
        debug!("getting current mouse location using CGEvent::new with a valid event source");

        // Obtain current mouse position using a newly created CGEventSource
        let source = CGEventSource::new(CGEventSourceStateID::HIDSystemState)
...
impl MacOSUIElement {
    ...
    fn click_auto(&self) -> Result<ClickResult, AutomationError> {
        // Check if we're dealing with a web browser application first
        // since mouse simulation works better on web content
        if let Some(app) = self.get_application() {
            // Get application name and check if it's a browser
            let app_name = app.attributes().label.unwrap_or_default().to_lowercase();
            debug!("detected application: {}", app_name);
            
            // Check against common browser names
        ...
        // For non-browser apps, follow the standard click approach
